...
describe('globalSearchGroups', () => {
  ...
  it('includes knowledge bases in all search and supports knowledge filtering', () => {
    ...
    expect(getGlobalSearchTypes('all')).toEqual(['topic', 'assistant', 'agent', 'knowledge-base'])
  ...
  it('maps topic filter to the topic search type and group', () => {
    const response: EntitySearchResponse = {
      query: 'plan',
      groups: [
        {
  ...
  it('collapses topic groups only in all search', () => {
    const response: EntitySearchResponse = {
      query: 'plan',
      groups: [
        {
