 ...
 *
 * `favorites` is the full ordered list of app favorites the sidebar renders and
 * drag-reorders as one list; `reorderFavorites` persists a new order. The launchpad
 * owns its own tile ordering elsewhere (built-in apps via `ui.launchpad.app_order`),
 * so favorites carries the sidebar order only. Every mutation goes through the
 * mix-preserving helpers in `utils/sidebar`, so components never touch the raw
 * `type` tags.
 */
